...
### Added
- `mcp-flow-server`: **Optimistic ack** option. When enabled, a tool call replies
  `ok` to the MCP client immediately after emitting `mcp-tool-execute`, without
  waiting for an `mcp-tool-response` from the flow (fire-and-forget; no response
  wiring needed). Disabled by default (server waits for the response, 30s timeout).

## [2.0.1] - 2026-06-07

### Changed
- Calmer node palette (muted purple / pink).
